...
public class OssFileServiceImpl extends ServiceImpl<OssFileMapper, OssFile> implements OssFileService {
    ...
    public FileVo uploadFile(MultipartFile file) {
        ...
        if (ossFile == null) {
            ...
            //上传过，引用计数+1
            //url失效，重新上传
            if (!UrlUtils.isUrlValid(ossFile.getAccLocation())) {
                //上传过，但是地址不可用了，重新上传
                String url = uploadToOss(file);
        ...
        BeanUtils.copyProperties(ossFile, fileVo);
